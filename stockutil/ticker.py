...
from numpy import append, dtype, frexp
import os
...
from util.utils import get_default_maxtry, get_date_list, get_target_date
...
class Ticker:
    ...
    def load_data(self,source):
        ...
        将取得的数据中 起止日期 和 init的输入的起止日期做比较，得到新的start date 和 end date。
        新的start 和 end 均为包含有数据的日子。
        """
        symbol = self.symbol
        self.data = None
        if source == "stooq":
    ...
    def get_date_lists(self, freq = 'W-WED', week_num = 2):
        """
        根据init时候确定的start 和 end，来得到 xyh 需要用到的日期列表。
        Parameters
        ----------
        freq： 定投的日子，默认每周三
        week_num: 每月定投的日子，默认是当月含有周三的第二周
        """
        start_date = self.start_date
        end_date = self.end_date
        self.date_lists = get_date_list(start_date, end_date, freq=freq, week_num= week_num)
        return True
    ...
    def get_price_list(self, date_list_name, get_maxtry =get_default_maxtry): 
        ...
        Parameters
        ----------
        date_list_name: 不同定投周期的代号，目前是 xmm、dmm
        get_maxtry: 若给定日期没有交易数据，需要向后尝试的天数。
        """
        price_list = []
    ...
    def cal_profit(self, date_list_name):
        ...
        Parameters
        ----------
        date_list_name : 不同定投周期的代号，目前是xmm 或者 dmm。 
        """
        if date_list_name not in self.date_lists.keys():
